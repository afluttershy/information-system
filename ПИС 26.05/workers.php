        ...
        <div class="list-group">
            ...
            <form class="form-inline">
                <div class="input-group mb-2 mr-sm-2">
                  ...
                  <input type="text" class="form-control" id="inlineFormInputGroupUsername2" placeholder="Имя сотрудника">
                </div>
              
                <button type="submit" class="btn btn-primary mb-2">Искать</button>
            </form>
            <a href="new-worker.php"><button type="button" class="addnewbtn btn btn-outline-success">Добавить нового сотрудника</button></a>
             <?php
				$host = 'std-mysql'; 
				$database = 'std_234'; 
				$user = 'std_234'; 
				...
				if ($result = $link->query($query)) {
					while ($row = $result->fetch_assoc()) {
						 echo '<a <a href="one-worker.php?F='. $row['id'].'"><button type="button" class="list-group-item list-group-item-action">';
						 echo '<p style="padding-right: 10px; color: #9932CC; display: inline-block">';
						 echo  $row['fio_worker'];
						echo '</p>';
						 echo '<p style="padding-right: 10px; display: inline-block"> Телефон: ';
						 echo  $row['tel'];
						echo '</p>';
